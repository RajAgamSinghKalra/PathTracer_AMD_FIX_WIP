...
#define WAVELENGTH_MIN 390
#define WAVELENGTH_MAX 830
...
vec3 CIE_CMF_XYZ(int lambda) {
    ...
    return spectralData.cie_cmf_xyz[int(lambda) - 390];
}
...
vec3 CIE_BT709_Basis(int lambda) {
    ...
    return spectralData.cie_bt709_basis[int(lambda) - 390];
}
